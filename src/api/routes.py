    ...
    async def event_generator():
        ...
        try:
            ...
            if answer_stream is not None:
                # answer_stream 是同步生成器，在线程中逐个取出 token
...
async def graph_overview(doc_id: str = "", limit: int = 200):
    """
    返回概念图谱数据（节点 + 边 + 社区），供前端 ReactFlow 渲染。
    ...
    try:
        ...
        with nebula.get_session() as session:
            ...
            if doc_id:
                concept_query = (
                    f'GO FROM "{doc_id}" OVER HAS_SECTION '
                    f"YIELD dst(edge) AS sid "
                    f"| GO FROM $-.sid OVER CONTAINS_CHUNK "
                    f"YIELD dst(edge) AS cid "
                    f"| GO FROM $-.cid OVER CONTAINS_CONCEPT "
                    f"YIELD DISTINCT dst(edge) AS concept_id, "
                    f"properties($$).phrase AS phrase, "
                    f"properties($$).community AS community "
                    f"| LIMIT {limit};"
                )
            else:
                concept_query = (
